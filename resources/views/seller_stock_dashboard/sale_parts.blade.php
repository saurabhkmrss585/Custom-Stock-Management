...
<body>
  ...
  <div class="form-container ">
    <h1>Sale Parts</h1>
    <form action="/sold_parts" method="post">
        @csrf
        <!-- Sale Date -->
        ...
        <h5>Select Parts</h5>
        <label for="componentSelect">Parts:</label>
        <select name="product_name[]" id="componentSelect" required>
            @foreach ($fetch_parts as $s)
                <option value="{{ $s->part_name }}" data-quantity="{{ $s->part_quantity }}" data-price="{{ $s->part_price }}">
                    {{ $s->part_name }}
                </option>
            @endforeach
        </select>
        ...
        <button type="button" id="addComponentBtn" class="btn">Add Parts</button>
    ...
    // Set the initial price for the first item
    updatePrice();
});

document.getElementById('componentSelect').addEventListener('change', updatePrice);
